...
The obtained dataset has been randomly partitioned into two sets, where 70% of the volunteers was selected for generating the training data and 30% the test data. The "data/train" folder in the downloaded dataset contains the training data, and the "data/test" folder contains all the test data.

The original downloaded data includes the following files:

- 'README.txt'
- 'features_info.txt': Shows information about the variables used on the feature vector.
- 'features.txt': List of all features.
- 'activity_labels.txt': Links the class labels with their activity name.
- 'train/X_train.txt' & 'test/X_test.txt': A 561-feature vector with time and frequency domain variables, corresponding to the measurements from the accelermoter & gyroscope.
- 'train/y_train.txt' & 'test/y_test.txt': Activity identifiers corresponding to the recorded activity.
...
The "run_analysis.R" script included in the repository performs these specific functions, which include transforming the original data in several ways:

- reads in training & test data
- retrieves names of features corresponding to the mean and standard deviation measurements
- cleans up feature names: removes/replaces '-' and '()'to make them more readable and less error-prone
- reads in activity labels to name the activities in the data set
- converts subjects & activities to factor
- merges data corresponding to the subjects, activity, features..
- combines training and test data into one big dataset
- assigns meaningful names to the variables in the combined data set
- melts resulting dataset to contain: subject, activity, variable, value
- transforms molten tidyData to contain average values for each of the 79 features, for each subject and activity
- writes resulting tidyData into a text file ("tidyData.txt")


## tidyData output

The resulting tidyData contains 180 observations and 81 variables.
The variables include the subject, the activity and mean values for the 79 features derived from accelermoter & gyroscope measurements.

An overall structure / summary of the tidyData output by run_analysis.R is provided below:
